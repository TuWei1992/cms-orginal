    ...
    $.imgAreaSelect = function(img, options) {
        ...
        function adjust() {
            ...
            if ($().jquery == '1.3.2' && $.browser.safari && position == 'fixed') {
                imgOfs.top += max(document.documentElement.scrollTop, $('.ImgBox').scrollTop());
                imgOfs.left += max(document.documentElement.scrollLeft, $('.ImgBox').scrollLeft())
            }
        ...
        function docMouseUp(event) {
            $('.ImgBox').css('cursor', '');
            if (options.autoHide || selection.width * selection.height == 0) hide($box.add($outer),
            function() {
                $(this).hide()
            });
        ...
        function areaMouseDown(event) {
            ...
            adjust();
            if (resize) {
                $('.ImgBox').css('cursor', resize + '-resize');
                ...
                $box.unbind('mousemove', areaMouseMove);
                $(document).mousemove(movingMouseMove).one('mouseup',
                function() {
                    options.onSelectEnd(img, getSelection());
        ...
        function doResize() {
            ...
            if (aspectRatio) if (abs(x2 - x1) / aspectRatio > abs(y2 - y1)) aspectRatioYX();
            else aspectRatioXY();
        ...
        var docKeyPress = function(event) {
            var k = options.keys,
            d, t, key = event.keyCode || event.which;
        ...
        $p = $img;
        while ($p.length && !$p.is('div.ImgBox')) {
