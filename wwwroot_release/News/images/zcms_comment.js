...
var Comment=(function(cmtConfig){
	...
	Event.removeEventListener=function(el,type,listener){
		if(document.addListener){
			el.removeListener(type,listener);
		}else if(document.attachEvent){
	...
	function initCommentMemberInfo(){
		...
		loadJs(url, function(){
			...
			if(login){
				//底部评论框显示的处理			
				if(loginContainer){
					loginContainer.innerHTML = login + '&nbsp;&nbsp;&nbsp;&nbsp;<a href="#;" onclick="Comment.commentLogout();return false;">'+localsForComment.Application_Logout+'</a>';			
					document.getElementById(elIds.isLogin).value="true";
				}
				//回复评论框的显示处理
				if(loginContainer_reply){
					loginContainer_reply.innerHTML = login + '&nbsp;&nbsp;&nbsp;&nbsp;<a href="#;" onclick="Comment.commentLogout();return false;">'+localsForComment.Application_Logout+'</a>';			
		...
		if(authCodeImg!=null){
			authCodeImg.style.background='url('+localsForComment.FrontAppContext+'authCode.zhtml?Height=21&Width=50&'+new Date().getTime()+') no-repeat center center';
		}
		if(authCodeImg_reply!=null){
			authCodeImg_reply.style.background='url('+localsForComment.FrontAppContext+'authCode.zhtml?Height=21&Width=50&'+new Date().getTime()+') no-repeat center center';
		}
	};
	...
	function commentLogout(){
		var url = localsForComment.FrontAppContext+'member/ajaxlogout?action=logout' + '&t=' + (+new Date());
		loadJs(url,function(){
			window.parent.location.reload();
		}); 
	}
	...
	//用户注册
	function register(){
		var url=localsForComment.FrontAppContext+'member/register?IsPreview='+localsForComment.Request_IsPreview+'&SiteID='+localsForComment.SiteID;
