...
var easyDialog = function(){
	...
	var	body = doc.body,
		...
		isIE7 = isIE && /msie 7/.test( navigator.userAgent.toLowerCase() ), // 判断IE7
		uuid = 1,
	...
	Dialog.prototype = {
		...
		getOptions : function( arg ){
			var i,
				...
				defaults = {
                    ...
                    success:    null,             //function          填充完内容后执行的回调函数
		...
		createDialogBox : function(){
			...
			dialogBox.style.cssText = 'margin:0;padding:0;border:none;z-index:10000;float:left;width:auto';
		...
		createDialogWrap : function( tmpl ){
			...
			//弹出层内容
			if(tmpl.content instanceof Object && tmpl.content.length>0){
				var createCon = tmpl.content.html();
				tmpl.content.remove();
			} else {
				var createCon = tmpl.content;
			}
			...
			var header = tmpl.header ? 
				...
				'<div class="easyDialog_content">',
					header,
					'<div class="easyDialog_text" id="easyDialog_text">' + createCon + '</div>',
					footer,
				'</div>'
				].join(''),
	...
	var extend = {
		open : function(){
            ...
            if(typeof options.success === 'function'){
                options.success.call( extend );
            }

			if( isIE6 || isIE7){
				doc.getElementById('easyDialogTitle').style.display = 'none';
				dialogWrap.style.display = 'block';
				var cWidth = doc.getElementById('easyDialog_text').offsetWidth;
				doc.getElementById('easyDialogTitle').style.width = cWidth+'px';
				doc.getElementById('easyDialogTitle').style.display = 'block';
			...
			// 防止select穿透固定宽度和高度
			if( isIE6 && !options.overlay || isIE7){
...
})( window, undefined ,window.ECar||(window.ECar={}));
