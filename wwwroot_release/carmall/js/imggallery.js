...
var galleryScroll = (function() {	
	...
	function _change(o) {
		...
		function setwid(index){
			...
			if(imgwrapW > allowWid){
				changeObjs[index].getElementsByTagName("img")[0].style.width = allowWid + "px";
			}
			
			var curimgwrapW = imgwrapW > allowWid ? allowWid : imgwrapW;
			var curimgwrapH=changeObjs[index].getElementsByTagName("img")[0].offsetHeight;
			
			
			if(imgwrapW!=0 && curimgwrapH!=0){
				miniSelector(o.changeObj_wrap)[0].style.width = curimgwrapW +"px";
				if(!o.imgMaxHeight){
					miniSelector(o.changeObj_wrap)[0].style.height = curimgwrapH +"px";
				}
			}
			
			//给查看大图a标签附链接
			var bigimgsrc=changeObjs[index].getElementsByTagName("img")[0].getAttribute("bigsrc");
			miniSelector(o.lookBigImgObj)[0].href=bigimgsrc;
		}
